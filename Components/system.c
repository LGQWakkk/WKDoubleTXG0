...
void radio_init(void)
{
    ...
    if(si24r1_check(M1) == RETURN_SUCCESS){
        ...
        // 设置信道
        si24r1_set_channel(M1, 60);//发射端M1作为发射模式 使用信道10
        // si24r1_set_channel(M1, 100);//接收端M1作为发射模式 使用信道100
    ...
    if(si24r1_check(M2) == RETURN_SUCCESS){
        ...
        // 设置信道
        si24r1_set_channel(M2, 100);//发射端M2作为接收模式 使用信道100
        // si24r1_set_channel(M2, 60);//接收端M1作为接收模式 使用信道10
...
////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(rf1_state == RF_STATE_TX_RUNNING){//处于发射模式
            ...
            // if(rf1_has_new_data_to_send){//有新数据待发送
        ...
        if(rf2_state == RF_STATE_TX_RUNNING){//处于发射模式
            ...
            // if(rf2_has_new_data_to_send){//有新数据待发送
